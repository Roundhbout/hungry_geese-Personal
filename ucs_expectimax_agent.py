...
def ucs(curr_pos, cols, rows, bad_positions, goal):
    frontier = [(curr_pos, 0)]
    explored = []
    while True:
        if len(frontier) == 0:
        ...
        curr = frontier.pop(0)
        curr_pos, curr_cost = curr
        explored.append(curr_pos)
        if curr_pos == goal:
            return curr_cost
        for next_pos in get_valid_successors(curr_pos, cols, rows, bad_positions):
            next_cost = curr_cost + 1
            next_node = (next_pos, next_cost)
            if next_pos not in explored:
                frontier_states = [node[0] for node in frontier]
                if next_pos not in frontier_states:
                    frontier.append(next_node)
                
                elif next_pos in frontier_states and next_cost < [cost for pos, cost in frontier if next_pos == pos][0]:
                    frontier.remove((next_pos, [cost for pos, cost in frontier if next_pos == pos][0]))
                    frontier.append((next_pos, next_cost))
    
def min_ucs_distance(pos, cols, rows, bad_positions, food):
    best_distance = None
    for nom in food:
        cur_distance = ucs(pos, cols, rows, bad_positions, nom)
...
"""
This is a searching agent using UCS and expectimax behavior. It checks for collision.
"""
def agent(observation, configuration):
    ...
    feasible_food = find_feasible_food(player_head, [goose[0] for goose in enemy_geese], food, board_cols)
    ...
    for action in Action:
        if last_action is None or action.opposite() != last_action:
            ...
            if neighbor not in bad_positions:
                ...
                if feasible_food:
                    dist = min_ucs_distance(neighbor, board_cols, board_rows, bad_positions, feasible_food)
                    if dist is not None:
